...
Using the definition of the osmotic pressure in Eq.~\eqref{eq:osmotic_pressure}, the surface tension can be simplified to
\begin{equation}
\gamma
=
\int_{-L/2}^{L/2} dx\,
...
Geometrically, the three coexisting phases define a common tangent plane that touches the free energy surface at three distinct points (orange triangle in Fig.~\ref{fig:three_phase_intuition}).
\noindent
...
\begin{minipage}{0.8\textwidth}
    ...
    \captionof{figure}{Free energy surface (gray contours) in composition space. The three-phase region (orange triangle) provides a better convexification of the free energy surface than the two-phase tie line (blue line).
    Therefore a three-phase decomposition always yields a lower free energy \(f_3\) than a two-phase decomposition \(f_2\).\\}
...
To see this, we will first use the lever rule to decompose a composition \(\boldsymbol{\phi}\) into two \(N_p=2\) or three \(N_p=3\) coexisting phases, according to
\[
\boldsymbol{\phi} = \sum_{i=1}^{N_p} \lambda_{i,N_p} \boldsymbol{\phi}^{(i,N_p)},
\qquad
\sum_{i=1}^3 \lambda_{i,N_p} = 1,
\qquad
\lambda_{i,N_p} > 0.
\]
For the three-phase decomposition, any point inside of the triangle must adheres to the following relation:
\begin{equation}
f(\boldsymbol\phi) \geq \sum_{i=1}^3 \lambda_{i,3} f(\boldsymbol{\phi}^{(i,3)}) \eqcolon f_3,
\label{eq:three_phase_free_energy}
\end{equation}
where the \(\lambda_{i,3}\) are the lever-rule coefficients for the three-phase decomposition.
This means that the free energy of the original composition is higher than the free energy of the three-phase decomposition.
By contrast, any two-phase decomposition yields a larger free energy, according to the following relation:
\[
f_2 \coloneq \sum_{i=1}^2 \lambda_{i,2} f(\boldsymbol{\phi}^{(i,2)}) \geq \sum_{i=1}^2 \lambda_{i,2}\sum_{j=1}^3 \bar\lambda_{ij,3} f(\boldsymbol{\phi}^{(j,3)}) = \sum_{j=1}^3 \lambda_{j,3} f(\boldsymbol{\phi}^{(j,3)}) = f_3.
\]
where in the second step applied the three-phase decomposition for each of the \(\boldsymbol\phi^{(i,2)}\) according to Eq.~\eqref{eq:three_phase_free_energy} and in the third step we defined use the relation
\[
\lambda_{j,3} = \sum_{i=1}^2 \lambda_{i,2} \bar\lambda_{ij,3}.
\]
Consequently, whenever a three-phase construction is available, it is energetically favored over any two-phase separation. A visual illustration of this point is provided in Fig.~\ref{fig:three_phase_intuition}
where the three-phase region (orange triangle) lays below the two-phase tie line (blue line) in terms of free energy.

...
Next, we look at two different classes of interaction matrices that emerge as we vary the parameter \(\delta\).
To investigate this behavior, we consider interaction matrices of the form:
\[
\tilde{\chi}=
\begin{pmatrix}
\chi+\delta& \chi-\delta\\
...
\end{pmatrix}
\]
where we fix $\chi<-4$ and vary $\delta$. This particular upper limit for \(\chi\) ensures that the system phase separates at the boundary of the phase diagram, which is a prerequisite for the first class of phase diagrams described below.\\
The first class of phase diagrams is characterized by the lack of critical points and the spinodal and binodal start and end at the boundary of the phase diagram (Fig.~\ref{fig:interesting_case}A-C).
...
When $\delta$ exceeds $-\chi-4$, the topology changes.
Here phase separation no longer reaches the boundary,
and we obtain a second class of phase diagrams in which both the binodal and the spinodal are fully contained within the admissible domain.
Instead of having no critical points, these diagrams exhibit two critical points on opposite sides of the phase diagram. For $\delta\gg|\chi|$, the overall shape of the phase diagram changes only weakly (Fig.~\ref{fig:interesting_case}E,F).
...
\begin{minipage}{0.95\textwidth}
    ...
    \captionof{figure}{Phase diagram for \(\chi=-5\) and varying \(\delta\). (A) \(\delta=-0.5\), (B) \(\delta=0.0\), (C) \(\delta=0.5\), (D) \(\delta=1.1\), (E) \(\delta=10.0\) and (F) \(\delta=60.0\).\\}
    \label{fig:interesting_case}
\end{minipage}\\

A feature seen in both classes is that the vertical tie-line at the center of the phase diagram is the same for all values of $\delta$. Using the ansatz \(\phi_1 = \phi_2 = \phi\), the free-energy density simplifies to
\[
 2\phi \ln(\phi) + (1-2\phi) \ln(1-2\phi) + 2\chi \phi^2.
\]
Thus, the coexistence conditions along this tie-line are independent of $\delta$ and depend only on $\chi$. For \(\chi=-5\), this yields the coexistence compositions \(\phi_{1/2}^{(1)} \approx 0.072\) and \(\phi_{1/2}^{(2)} \approx 0.428\).

...
A tentative measure of the degree of interfacial adsorption is the displacement between the centers of the fitted profiles of components 0 and 1, which we denote by \(\Delta x_0\).
...
In contrast, when \(\Delta x_0 \gg w\), a third phase develops in the interfacial region, corresponding to the opposite limit of interfacial adsorption.\\
...
One thing to note is that even when the displacement between the centers of the profiles nearly vanishes, the relative error does not go to zero but instead approaches a finite value.
...
In this section we will consider the symmetric case, which corresponds to \(\chi_{01}=\chi_{02}\).
The symmetric case has the advantage over the general case, that one can exploit the symmetry to derive analytical expressions for key interfacial properties, which corresponds.
This will allow us to gain a deeper understanding of the interfacial behavior and the sources of error in the piecewise-linear approximation.\\
...
Here species 0 can become adsorbed at the interface which follows earlier work on interfacial adsorption~\cite{Huang1996,Huang1999}, in contrast to the earlier convention where species 2 played this role,
...
\begin{minipage}{\textwidth}
    ...
    The analytical solution becomes exact near the critical point. Surprisingly, the relative error seems to vanish near the binary-mixture limit \((\phi_0 \to 0)\),

...
For this fully symmetric system, previous work \cite{Huang1995PhaseSeparation} provides analytical results that can be used to verify our algorithm, at least qualitatively, since the results are presented only in figures.
...
Finally at \(\chi=2.746\) the three three-phase regions join to a single three-phase region (Fig.~\ref{fig:symmetric_chis}E)\footnote{Note that the critical points still exist within the three phase region.}.
...
Apart from the symmetric case there are some other interesting cases, at least from a phenomenological perspective.
One such case is a system where the spinodal and binodal sections span the phase diagram but do not meet, meaning that there is no critical point.
...
This makes sense if one considers that for $\delta<0$ the cross-species interaction is more repelling than the same-species interaction. Thus if we start at the dilute limit and the solvent is added (i.e. we move towards the center of the phase diagram) the repelling force is damped and phase separation is weaker.
If $\delta=0$ cross-species interactions are the same as same-species interactions and thus the binodal/spinodal are straight.
...
In this class the binodal/spinodal is fully contained inside the admissible domain and does not intersect the boundaries. 
Instead of no critical points there are now two critical points on opposite sides of the phase diagram. 
For $\delta\gg|\chi|$ the phase diagram barely changes its shape. This is because the entropic part becomes negligible in the free-energy density compared to the enthalpic part, which is dominated by $\delta$.
In this limit the free-energy density is approximately given by
\[
\begin{aligned}
...
\end{aligned}
\]
and thus, by changing $\delta$, we only change the overall scale of the free-energy density, which does not affect the coexistence conditions.
...
We can find the tie-line given the ansatz \(\phi_1 = \phi_2 = \phi\). Thus the free energy simplifies to
\[

...
Phase separation in multicomponent fluids provides a general physical framework for understanding how a homogeneous mixture can spontaneously demix into coexisting phases with distinct compositions.
In biological systems, this framework has become especially important because such demixing can give rise to biomolecular condensates that compartmentalize cellular matter without a surrounding membrane, thereby creating distinct biochemical environments \cite{Kilgore2023}.
Such condensates are implicated in diverse biological contexts, including development \cite{So2021}, gene regulation \cite{Pei2025}, protein quality control \cite{Rajendran2025}, and signalling \cite{Jaqaman2021}. They are also linked to pathological states such as neurodegeneration \cite{Mathieu2020} and cancer \cite{Cai2021BiomolecularCA}, and are found in the nucleus \cite{Lafontaine2020}, cytosol \cite{Banani2017}, and at membranes \cite{Jaqaman2021}.
A theoretical description of these systems must therefore bridge multiple length scales, from microscopic molecular interactions that drive phase separation \cite{Dignon2020} to macroscopic interfacial phenomena such as Marangoni flows \cite{Michelin2023} and wetting \cite{Cahn1977}, which govern condensate behavior at the cellular scale.

Compared with binary mixtures, ternary systems already display a much richer phenomenology. Their phase diagrams are two-dimensional, tie-lines can vary across composition space, three-phase coexistence can occur \cite{Scott1949}, and interfaces may exhibit preferential adsorption of one component \cite{Huang1996AnalyticInterface}.
These features make ternary mixtures the simplest setting in which genuinely multicomponent effects can arise.
They are also of practical interest, since many biological and synthetic mixtures contain multiple species whose interactions cannot be reduced to an effectively binary description \cite{VEATCH20033074,Kaur2021}. Ternary phase separation has therefore attracted considerable theoretical attention, for example in the context of chemically regulated condensates \cite{Kirschbaum2021Controlling} and self-propulsion in droplets \cite{Demarchi2023EnzymeEnriched}.

Two complementary questions are central to ternary phase separation. First, which bulk states can the system realize? Phase diagrams answer this by identifying coexistence regions, critical points, tie-lines, and possible three-phase equilibria. They therefore provide the global map needed to predict how a mixture responds to changes in composition or interaction strength.
Several methods have been developed to reconstruct binodals directly from interaction matrices, including analytic approaches, genetic algorithms, and constructions based on convexification of the free-energy landscape \cite{Milani2007,WEI200691,Mao2019}.

Second, how are coexisting phases joined by interfaces?
This question is addressed by interfacial physics.
The structure and free-energy cost of an interface influence nucleation barriers, relaxation dynamics, and the stability of phase-separated states \cite{Philippe2015,zwicker2025physicsdropletregulationbiological}. Additionally, composition-dependent surface tension can drive Marangoni flows along interfaces, leading to spreading and contraction dynamics in ternary droplets \cite{Baumgartner2022}. In ternary mixtures, interfaces are particularly interesting because one component can adsorb preferentially at the interface, thereby altering the surface tension and the interfacial structure. Classic studies of symmetric ternary mixtures showed that these interfacial properties can differ qualitatively from their binary counterparts \cite{Huang1996,Huang1996AnalyticInterface,Huang1999}.

These two perspectives, global phase behavior and local interfacial structure, are naturally linked.
Phase diagrams identify the relevant coexisting bulk phases, while interfacial calculations characterize the boundaries between them.\\

This thesis focuses on these two complementary aspects within ternary Flory-Huggins theory. First, we investigate the structure and energetics of diffuse interfaces in Chapter~\ref{s:surface_tension}. We are particularly interested in the role of interfacial adsorption and in analytical approximations for the surface tension.
Second, we develop a continuation-based numerical method for the rapid construction of ternary binodals and phase diagrams directly from the coexistence conditions in Chapter~\ref{s:binodal_tracing}.
Together, these two chapters connect bulk coexistence with interfacial structure, providing a unified picture of ternary phase separation within Flory-Huggins theory.

% The remainder of this thesis is organized as follows. In Chapter 2, we develop the theoretical framework for incompressible ternary mixtures, including the thermodynamics of phase coexistence, the Flory-Huggins free-energy functional, the coupled Cahn-Hilliard dynamics, the thermodynamic definition of surface tension, and the numerical methods used to compute equilibrium profiles. In Chapter~\ref{s:surface_tension}, we then study the surface tension of ternary interfaces numerically and analytically, and show that preferential interfacial adsorption is a key correction to simple profile approximations. Finally, in Chapter~\ref{s:binodal_tracing}, we introduce a fast continuation-based algorithm for tracing ternary binodals and demonstrate that it accurately captures a wide range of phase-diagram topologies.
...
Minimizing the free-energy in Eq.~\eqref{eq:free_energy_two_phase} with respect to these three variables yields the coexistence conditions
\begin{equation}
    \begin{aligned}
...
\begin{minipage}{0.5\textwidth}
    ...
    and the regions of stability, metastability and instability of the homogeneous state as well as the critical point (gold star).  }
...
\]
the interaction energy in Eq.~\eqref{eq:ising_hamiltonian} takes the form \textcolor{red}{Perhaps add the full calculations in the appendix?}
\[
E
=
\frac{1}{a}\int dx\,
...
The free-energy functional in Eq.~\eqref{eq:fh_free_energy} contains gradient terms, that penalize sharp spatial variations and thereby stabilize smooth interfaces between coexisting phases. For this penalty to be non-negative, the gradient matrix of the reduced system
\[
\tilde{\kappa}=
\begin{pmatrix}
...
In the following, we restrict ourselves to systems in which diffusion dominates over advection, so that the advective term in Eq.~\eqref{eq:continuity_equation} can be neglected. This approximation is valid for systems with a small Péclet number,
\[
Pe=\frac{Lv}{D}\ll 1,
\]
...
Substituting Eq.~\eqref{eq:flux_mobility} into Eq.~\eqref{eq:continuity_equation} yields the general form of the Cahn-Hilliard equations for a ternary mixture in the absence of hydrodynamics and thermal fluctuations:
\begin{equation}
\frac{\partial \phi_i}{\partial t}
=
\nabla \cdot
...
In the following, we present a thermodynamic approach for deriving the surface tension of a ternary system and then showcase the explicit form for our model.

\subsubsection{Thermodynamic derivation}

In this section, we derive the surface tension of a ternary system from thermodynamic principles. A more detailed derivation can be found in \cite{Ip1994}.
...
For a planar interface, the surface tension is given by Eq.~\eqref{eq:surface_tension_thermodynamic}.
...
\qquad i\in\{1,2\}.
\]
Inserting the Flory-Huggins free-energy density from Eq.~\eqref{eq:fh_free_energy_density} yields the coupled second-order equations
\[
\begin{aligned}
...
To solve the fixed-point iteration in Eq.~\eqref{eq:fixed_point_iteration}, we employ a pseudo-spectral method.
...
Inserting this into the fixed-point iteration scheme from Eq.~\eqref{eq:fixed_point_iteration} and rearranging yields
\begin{equation}
    \phi_i^{n+1}
=
\phi_i^n
...
We declare convergence when the maximum change of the fields \(\boldsymbol\phi^n(x) =(\phi_1^n(x)\,\phi_2^n(x))^T\) between two iterations satisfies
\[
...
Additionally, the critical exponent is consistent with previous work \cite{Huang1996AnalyticInterface,Anisimov2024CriticalFluctuations}. In contrast, the prefactor of the power law
...
\begin{minipage}{\textwidth}
    ...
    \captionof{figure}{(A) Exact (red) and approximate (blue) surface tension for \(\chi_{12}=1.4\), \(\chi_{02}=1.5\), \(\chi_{01}=2.5\)
...
\begin{minipage}{0.85\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/factor_scaling.png}
...
In this section, we consider the symmetric case, defined by \(\chi_{02}=\chi_{12}\).
...
The interaction matrix of the reduced two-component system can be obtained from the original three-component interaction matrix according to Eq.~\eqref{eq:reduced_chi}, which for the symmetric case reduces to\\
...
\begin{aligned}
    ...
    &-\chi_o \phi_0''(x) - \chi_d  \phi_1''(x) = \chi_o \phi_0 + \chi_d \phi_1 + \ln(\phi_1) - \ln(1-\phi_0-\phi_1)-\mu_1\\
\end{aligned}
\]
...
\[
\gamma_\mathrm{approx}= \sqrt{(\Delta \phi_i\kappa_{ij} \Delta \phi_j)\,(f(\phi_+(0)) -2\mu \phi_+(0)+ \Pi)}
\]
where now instead of using the mean composition \(\bar\phi\) we use the composition at the interface \(\phi_+(0)\) including the effect of interfacial adsorption.
...
\begin{minipage}{0.95\textwidth}
    ...
    With \(\chi=2.5\), the color scale indicates the interaction strength \(\eta\).\\}
...
Since \(\boldsymbol{\phi}\) lies on the binodal, we have \(\boldsymbol{E}(\boldsymbol{\phi})=\boldsymbol{0}\). Remaining on the binodal to first order then requires
\[
...
\]
The dimension of the null space of the Jacobian is generally one-dimensional, since it is equal to number of columns minus the rank of the matrix
\[
\dim(\operatorname{null}(\boldsymbol{J})) = 4 - \operatorname{rank}(\boldsymbol{J}) = 1,
\]
so there is a unique local direction along which one can step.
...
Linearizing the coexistence conditions as in Eq.~\eqref{eq:binodal_step} gives
\begin{equation}
\boldsymbol{E}(\boldsymbol{\phi}+\delta\boldsymbol{\phi}_t)
=
...
The predictor step has previously been used for this purpose \cite{Shcherbakova2023}.
In the present implementation, however, each tangential step is followed by a projection back onto the coexistence manifold.
This predictor-corrector strategy permits larger step sizes while maintaining accuracy, enabling efficient and accurate tracing of the binodal (Fig.~\ref{fig:tracer_schematics}A).
...
For the Flory-Huggins free-energy density, both types of initial points can be constructed efficiently, making them well suited for initializing the algorithm.
...
Because the initial point lies very close to the boundary of the composition space, applying the full projection step can occasionally move the system outside of the physically admissible domain.
...
Because the binodal and spinodal meet at the critical point, the tangent direction in composition space can instead be inferred from the null vector of the Hessian of the free-energy density (see Eq.~\eqref{eq:hessian_null_space}).
...
We identify three-phase regions by searching for intersections between distinct binodal sections (see Fig.~\ref{fig:tracer_schematics}D). Concretely, we look for pairs of coexistence states, \(\boldsymbol\phi^{\,1} = (\phi_1^{(1)}, \phi_2^{(1)}, \phi_1^{(2)}, \phi_2^{(2)})^T\) and \(\boldsymbol\phi^{\,2} = (\phi_1^{(1)}, \phi_2^{(1)}, \phi_1^{(3)}, \phi_2^{(3)})^T\), that share one phase but differ in the second one, i.e. \(\phi_i^{(2)} \neq \phi_i^{(3)}\) for at least one \(i\). Such an intersection identifies a composition that coexists with two distinct phases and therefore forms the common endpoint of two tie-lines.
This should be distinguished from a branching point, which corresponds to a single coexistence state from which two continuation directions emerge.

If one phase coexists with two others in this way, then all three compositions satisfy the same coexistence conditions and therefore represent three coexisting phases.
...
\begin{minipage}{0.95\textwidth}
    ...
    \captionof{figure}{(A) Ternary diagram, with axes \(\phi_0\) on the right diagonal, \(\phi_1\) on the left diagonal and \(\phi_2\) on the horizontal axis. The red star is at \((\phi_0\,\phi_1\,\phi_2)^T=(0.2\,0.5\,0.3)^T\) and the green star is at \((\phi_0\,\phi_1\,\phi_2)^T=(0.9\,0.1\,0.0)^T\). Note that the sum of the three volume fractions is always 1.\\
     (B) Phase diagram for \((\chi_{01},\chi_{02},\chi_{12})=(2.6,2.9,2.7)\), showing the spinodal (thick dashed line), binodal (blue line), three-phase region (orange areas), critical points (gold stars) and tie-lines (gray dashed lines).}
...
The symmetric ternary case is defined by equal interactions of one species with the other two, \(\chi_{02}=\chi_{12}\), while the interaction between the remaining two species is given by \(\chi_{01}\).
For the purpose of this example, we assume species 2 to interact equally with species 0 and 1. Of course, the same construction applies to the other species after relabeling the species and the corresponding interaction parameters.
...
\begin{aligned}
\phi_2^{(1)}& = \phi_2^{(2)},\\
\phi_0^{(1)}&= \phi_1^{(2)}\eqcolon \phi^{(1)},\\
\phi_0^{(2)}&= \phi_1^{(1)}\eqcolon \phi^{(2)}
\end{aligned}
\]
...
\end{equation}\\
Thus, for a given \(\phi^{(1)}\), we can find \(\phi^{(2)}\) by solving the implicit equation Eq.~\eqref{eq:symmetric_implicit}.
...
The figure is shown for a particular choice of \(\chi_{01}\), but the same level of agreement was obtained for other values of \(\chi_{01}\in[2,3]\).
...
\begin{minipage}{0.5\textwidth}
    ...
    \captionof{figure}{Top: Binodal curve for the symmetric ternary system with \(\chi_{02}=\chi_{12}=1.5\) and \(\chi_{01}=2.5\). The dashed line shows the analytical solution from Eq.~\eqref{eq:symmetric_implicit}, while the blue and orange points show the results obtained with our algorithm. Bottom: The residual between the analytical and numerical results is negligible.\\}
...
In their work, the authors consider several interaction regimes and find a rich variety of phase-diagram topologies for \(\chi \in [2, 3]\) which they present in the first figure of \cite{Huang1995PhaseSeparation}.
...
Overall, the sequence of topological changes obtained from our algorithm matches \cite{Huang1995PhaseSeparation} exactly.\\
\begin{minipage}{0.95\textwidth}
    \centering
...
Although the two chapters treat different aspects of ternary phase separation, together they follow the natural order in which ternary systems are usually analyzed. The phase-diagram algorithm enables the rapid exploration of the phase behavior over a wide range of ternary systems. It can therefore be used to identify promising candidates for the subsequent study of interfacial phenomena.\\

Several natural directions for future work follow from these results. For the interfacial system, we begin with a number of smaller problems that would serve more as extensions of the present thesis. First, it would be interesting to derive the critical exponent of \(3/2\) directly from the exact definition of the surface tension given in Eq.~\eqref{eq:surfacetension_flory}, in order to corroborate the results of the simplified approximation. Second, it would be interesting to validate the ans\"atze in Section~\ref{sss:profile_approximation} for the profiles in the symmetric ternary case. For \(\phi_-\), this would likely involve expanding the Euler-Lagrange equation in Eq.~\eqref{eq:el_symmetric} around the critical point and showing that the resulting equations are solved by the hyperbolic-function ansatz (following a procedure similar to that used in \cite{Getzinger2026}). For \(\phi_+\), one would neglect the second-derivative term in the Euler-Lagrange equation, as was done for the implicit Eq.~\eqref{eq:delta_phi_plus}, and show that the resulting equation is solved by the hyperbolic-function ansatz with the adsorption peak amplitude given by Eq.~\eqref{eq:delta_phi_plus}.
...
A possible strategy would be to introduce additional constraints thereby tracing lower-dimensional slices of the coexistence manifold. For example, suppose one is interested in the two-phase coexistence of a four-component incompressible system. In that case, there are \((N_c-1)\cdot 2 = 6\) degrees of freedom and \(N_c = 4\) coexistence conditions. If one then fixes the mean volume fraction of the solute, this introduces an additional constraint that reduces the coexistence manifold to a one-dimensional curve, which can be resolved using the current implementation of the algorithm. Alternatively, one could focus on higher-phase coexistence problems for which the manifold becomes one-dimensional again. For instance, in a four-component system with three-phase coexistence, the manifold again is one-dimensional.

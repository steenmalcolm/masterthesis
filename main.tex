...
If \(b\leq 1\), or equivalently \(\bar\phi \leq \phi_c\), the only root is \(x^*=0\).
Therefore phase separation can only occur for $\bar\phi > \phi_c$.
In addition, the positive root \(x^*>0\) satisfying \(x-b\tanh(x)=0\) increases with \(b\), meaning that as \(\bar\phi\) increases, so does \(\Delta \phi\).
The largest possible value for \(\Delta \phi\) occurs in the binary-mixture limit \(\phi_0 \to 0\), where \(\bar\phi = 1/2\),

...
The natural conclusion is therefore that additional binodal branches must exist and enclose the spinodal.

A useful diagnostic emerged from the singular values of the Jacobian along the binodal.
...
When such a degeneracy is detected, the continuation is split and both directions are followed in order to trace all branches of the binodal.\\
...
Such intersections signal the presence of points in phase space where two tie lines go in different directions,
which in turn indicates the existence of a three-phase region. (see Fig.~\ref{fig:tracer_schematics}D)\\
...
\end{minipage}\hfill
\\The thermodynamic meaning of this construction is as follows. If a point lies inside a three-phase region, it can be represented as a linear combination of three coexisting compositions.
Geometrically, this corresponds to a common tangent plane that touches the free energy surface at three distinct points (see Fig.~\ref{fig:three_phase_intuition}).
Although some points in this part of phase space may also admit a two-phase decomposition, such constructions provide only a partial convexification of the free energy surface, whereas the tangent plane associated with three-phase
coexistence yields a lower free energy, as it represents the full convex envelope in this region.
Consequently, whenever a three-phase construction is available, it is energetically favored over any two-phase separation
and the system will partition into three coexisting phases (see Fig.~\ref{fig:three_phase_intuition}).

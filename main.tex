...
It can accumulate at phase interfaces and therefore partially screen the direct repulsion between components 0 and 1.
...
The coexisting phases then are used to initialize the interface of our simulation as two step functions, which ensures periodic boundary conditions (see Fig.~\ref{fig:profiles_init}).\\
...
Starting from the initial step profile, we relax the system to equilibrium using the fixed-point iteration in Eq.~\eqref{eq:fixed_point_iteration}. Because we are only interested in the equilibrium profiles, the step size \(h\) is chosen as large as numerical stability permits. The semi-implicit update strongly suppresses high-wavenumber modes, allowing us to use \(h=\mathcal{O}(10^{-1})\), typically one to two orders of magnitude larger than in a fully explicit scheme.\footnote{Close to the phiysically admissable range of the volume fractions, smaller step sizes are required because the logarithmic terms in the free energy become large and can lead to numerical instabilities.}

We declare convergence when the maximum change of the volume fractions between two iterations satisfies
\[
...
After obtaining the equilibrium profiles, we compute the surface tension using the thermodynamic relation derived in Section~\ref{ss:surface_tension}.
In this way, we obtain surface-tension values along the binodal for different \(\bar\phi_2\), or equivalently for different values of the chemical potential \(\mu_2\) of species 2. Along the binodals considered here, \(\bar\phi_2\) and \(\mu_2\) are in one-to-one correspondence.

The resulting surface tensions are shown in Fig.~\ref{fig:surface_tension_numerical}.
...
To derive the approximation for the surface tension, it is useful to
...
The second-order term in the Taylor expansion of the chemical potential reads:
\begin{equation}
 \boldsymbol{\mu}^{(1/2)} - \boldsymbol{\mu}_c
    =
    \frac{1}{2}
...
The dot product of the chemical potential with the phase-composition difference can then be written as
\begin{equation}
    \begin{aligned}
   ...
   &=\frac{1}{2}\mu_{i,c}\Delta \phi_i
    ...
    \right]\\
    &+ \frac{1}{2^4 \cdot 3!}\,
     \frac{\partial^3 \mu_i}{ \partial \phi_j \partial \phi_k \partial \phi_l}\,
     \Delta \phi_i
...
\begin{minipage}{0.95\textwidth}
    ...
    for various interaction parameters (different colors). The dashed lines indicate the expected scaling with exponents \(1\) and \(2\) respectively.\\\newline}
...
\textcolor{red}{branching bei chi = 2.565 ist eine aenderung der Anzahl an Loesungen von nicht linearem System}
...
\begin{minipage}{0.5\textwidth}
    ...
    \captionof{figure}{Comparison of three phase coexistence compositions obtained from our algorithm and the \texttt{flory} package for points in the three-phase region (orange area). The green points correspond to correct predictions from the \texttt{flory} package, while the red crosses indicate cases where two-phase compositions were found that yield a slightly higher free energy.\textcolor{red}{Add legend and fill entire textwidth}}
...
The symmetric ternary case is defined by equal interactions of one species with the other two, \(\chi_{01}=\chi_{02}\)\footnote{For the purpose of this example, we assume species 0 to interact equally with species 1 and 2. Of course, the same construction applies to the other species after relabeling the species and the corresponding interaction parameters.}, while the interaction between the remaining two species is given by \(\chi_{12}\).
...
The results are shown in Fig.~\ref{fig:verify_symmetric}. In the bottom panel, the difference between the exact and algorithmic
...
For this fully symmetric system, previous work \cite{Huang1995PhaseSeparation} provides results that can be used to verify our algorithm, at least qualitatively, since the results are presented only in figures.
In that work, the authors consider several interaction regimes and find a rich variety of phase-diagram topologies for \(\chi \in [2, 3]\).
Here we reproduced those results using our algorithm and found good qualitative agreement, for the different regimes as we will describe now.\\
\end{minipage}
\hfill
\begin{minipage}{0.5\textwidth}
    \centering
...
\footnote{Note that the critical points still exist within the three phase region.}.
As one increases \(\chi\) further, the three-phase region grows in size (Fig.~\ref{fig:symmetric_chis}F).
Overall, the sequence of topological changes obtained from our algorithm is in good qualitative agreement with the results reported in \cite{Huang1995PhaseSeparation}.\\
\begin{minipage}{0.95\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/symmetric_chis.png}
    \captionof{figure}{Phase diagram for different values of \(\chi\).
    The orange triangles are the three-phase regions, the binodal is shown in blue, the spinodal is the thick dashed line and the critical points are the gold stars and the tie-lines are the thin striped lines.
    (A) \(\chi=2.4\), (B) \(\chi=2.65\), (C) \(\chi=8/3\), (D) \(\chi=2.7\), (E) \(\chi=2.746\), (F) \(\chi=3.0\). \textcolor{red}{Maybe add legend}}
...
Here, we turn to two different classes of phase diagrams that emerge as the interaction asymmetry parameter \(\delta\) is varied in the reduced interaction matrix for components \(1\) and \(2\):
\[
\tilde{\chi}=
\begin{pmatrix}
\chi+\delta& \chi-\delta\\
...
This particular upper limit for \(\chi\) ensures that the system phase separates at the boundary of the phase diagram, which is a prerequisite for the first class of phase diagrams described below.\\
...
When $\delta$ exceeds $-4-\chi$, the topology changes and phase separation no longer reaches the boundary. We then obtain a second class of phase diagrams in which both the binodal and the spinodal are fully contained within the admissible domain.
In this second class, two critical points appear on opposite sides of the phase diagram. For $\delta\gg|\chi|$, the overall shape of the phase diagram changes only weakly (Fig.~\ref{fig:interesting_case}E,F).
In this limit, the entropic contribution becomes negligible compared to the enthalpic contribution proportional to $\delta$, and the free-energy density is approximately given by
\[
\begin{aligned}
...
\end{aligned}
\]
Thus, changing $\delta$ only rescales the free-energy density and therefore does not affect the coexistence conditions.

\begin{minipage}{0.95\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/interesting_case.png}
    \captionof{figure}{Phase diagram for \(\chi=-5\) and varying \(\delta\in[-0.5,60.0]\). Blue line denote binodals, black dashed lines denote spinodal. (A) \(\delta=-0.5\), (B) \(\delta=0.0\), (C) \(\delta=0.5\), (D) \(\delta=1.1\), (E) \(\delta=10.0\) and (F) \(\delta=60.0\).\\}
    \label{fig:interesting_case}
\end{minipage}\\

A common feature of both classes is that the vertical tie-line at the center of the phase diagram is the same for all values of $\delta$. Using the ansatz \(\phi_1 = \phi_2 = \phi\), the free-energy density simplifies to
\[
...
Taken together, these results connect local interfacial physics to the global organization of phase space and provide a coherent framework for studying three-component liquid-liquid phase separation.\\

In the chapter on surface tension, the numerical calculations showed that preferential interfacial adsorption of the weakly interacting component is a central feature of ternary interfaces.
Close to the critical point, the surface tension vanishes with the mean-field power law \(\gamma \sim |\boldsymbol{\mu}_c-\boldsymbol{\mu}|^{3/2}\), in agreement with the expected critical scaling.
A simple analytical approximation based on linearly interpolating between the two coexisting phases within a common interfacial window already captures the qualitative behavior of the numerical results and,
importantly, reproduces the correct scaling near criticality.
At the same time, the comparison with the numerical profiles makes clear that the main deficiency of this approximation is the neglect of interfacial adsorption, which becomes increasingly important close to the critical point.

The symmetric ternary case made it possible to analyze this effect in more detail.
There, implicit expressions were obtained for the adsorption peak amplitude and the interfacial width, and a hyperbolic-function ansatz provided an accurate representation of the equilibrium profiles.
Further incorporating the adsorption peak amplitude into the analytical surface-tension estimate significantly improved the agreement with the numerical data, especially in the regime where the original approximation showed its largest relative error.
This strongly suggests that interfacial adsorption is the dominant correction, while the remaining discrepancy is likely due to the simplified shape of the approximate profiles.\\

The second main result of this thesis is the continuation-based algorithm for constructing ternary binodals.
The method combines tangential predictor steps with a projection back onto the coexistence manifold using the pseudoinverse of the Jacobian,
which makes it possible to trace the binodal efficiently and with high numerical accuracy.
During this development, several interesting geometric features of ternary phase diagrams became apparent, including branching points of the binodal, self-returning sections of the binodal,
and the appearance of three-phase regions through intersections of binodal sections. Validation against the \texttt{flory} \cite{Yicheng2025flory} package and against analytical results for symmetric cases
showed very good agreement. The algorithm also reproduced the rich topological changes of the fully symmetric ternary phase diagram and revealed additional classes of phase diagrams,
including cases without critical points and the robust vertical tie-line that is independent of the asymmetry parameter.

Several natural directions for future work follow from these results.
On the interfacial side, it would be interesting to try and derive the critical exponent of \(3/2\) directly from the exact definition of the surface tension,given in Eq.~\ref{eq:surface_tension_definition}, rather than from the simplified approximation.

Another interesting endeavour would be to validate the ans\"atze for the profiles in the symmetric ternary case.
For \(\phi_-\) this would likely involve expanding the Euler-Lagrange equations around the critical point and showing that the resulting equations are solved by the hyperbolic-function ansatz. 
For \(\phi_+\) one would need to neglect the second derivative term in the Euler-Lagrange equation and show that the resulting first-order equation is solved by the hyperbolic-function ansatz with the adsorption peak amplitude given by Eq.~\ref{eq:adsorption_peak_amplitude}.
Building on this, it would be interesting to extend the analytical treatment of Section~\ref{sec:symmetric_case} beyond the symmetric case,
for example through a perturbative expansion around symmetry. This may reveal ans\"atze for the profiles for non-symmetric systems, something that has so far only been obtained in very specific limits \textcolor{red}{cite paper from literature discussion}.

On the phase-diagram side, a simple extension is to apply the algorithm to other free-energy densities, for example Ginzburg-Landau free-energy. This would only require changing a single function since the calculus for for example the Jacobian is done automatically using JAX.
A harder challenge is the extension to systems with more than three components, where the coexistence manifold is no longer one-dimensional and can no longer be followed by a single continuation direction.
This would require constructing a mesh over the coexistence manifold, which is a much more complex task programmatically and numerically than the one-dimensional tracing performed here.
A possible strategy of avoiding this complexity would be to introduce additional constraints and trace lower-dimensional slices of the coexistence manifold. 
For example lets say we interested in the two-phase coexistence of a four component incompresible system.
We have \((N_c-1) \cdot 2 = 6\) degrees of freedom, and \(N_c = 4\) coexistence conditions.
If we then set the mean volume fraction of the solute to be constant, we introduce an additional constraint that reduces the ceoxistence manifold to a one-dimensional curve that can be traced using the continuation method developed here.
One could alternatively focus on higher-phase coexistence problems for which the manifold becomes one-dimensional again.
For example for a four-component system with three-phase coexistence, the manifold is one-dimensional again.
A different extension of the continuation framework could be to include the interaction parameters themselves as degrees of freedom, that can be varied during the continuation process.
For the ternary case, this would mean that there are now 7 degrees of freedom, such that the null space of the Jacobian is four-dimensional.
By investigating the eigenspectrum of the null-space of the Jacobian, one could then identify the direction in interaction-parameter space that induces the largest change in the coexistence compositions.
In other words, one could quantify how sensitive a coexistence state is to external perturbations.

A final direction for future work would be to investigate higher-component systems but treat it as a ternary system by labeling the remaining species as hidden species.
Again the null space of the Jacobian would be higher-dimensional, such that one could move the binodal of the ternary system in any direction by changing the composition of the hidden species.
This would in theory allow one to engineer the binodal curve to have a desired shape.

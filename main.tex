...
Once the interaction parameters are specified, the spinodal curve and the critical points are computed first. For the Flory-Huggins free-energy density, both can be obtained analytically. Together with the dilute-limit constructions described above, these determine the initial continuation tasks from which the binodal branches are traced. Here, a continuation task is the pair \((\boldsymbol{\phi}, \delta\boldsymbol{\phi}_t)\), consisting of a phase-space point \(\boldsymbol{\phi}\) and a continuation direction \(\delta\boldsymbol{\phi}_t\) that specifies which local branch to follow.

All initial continuation tasks are collected in a task list. The algorithm then proceeds by repeatedly selecting a task from this list and tracing the associated branch. During this tracing process, the singular values of the Jacobian are monitored in order to detect branching events. Whenever the second smallest singular value falls below a prescribed threshold, this indicates that an additional branch may emanate from the current point. In that case, a new continuation task is created from the current phase-space point together with the continuation direction that is not being followed by the current branch and this task is added to the task list.

In this way, the algorithm gradually discovers and explores all connected branches of the binodal. After the branches initialized from the dilute limits and critical points have been processed, the remaining branches are generated automatically through the detected branching events. The procedure continues until the task list is empty. To avoid tracing the same branch multiple times, previously explored continuation tasks are cached and duplicate tasks are skipped when encountered.
...
\While{$\mathrm{task\_list}$ is not empty}
    ...
    \ForAll{$q \in \mathrm{branch\_tasks}$}
...
\Function{ComputeBinodalSection}{$\mathrm{task}_0$}
    ...
    \While{termination criteria are not met}
        \State Take tangential step from $p$ along $d$ and project back to binodal

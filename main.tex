...
Next we derive the interface width, \(w = \Delta \phi /  \phi_{-}'(0)\), which differs from the earlier approximation in Eq.~\eqref{eq:interface_width_approximation} because adsorption effects are now included.
...
\end{aligned}
\]
where \(f_\mathrm{local}\) is the local free-energy density evaluated at the interface and \(\Pi\) is the osmotic pressure.
...
The choice of the hyperbolic secant squared for the excess part of \(\phi_+\), i.e. \(\phi_+(x)-\bar\phi\), is motivated by the fact that this function is symmetric and decays to zero in the bulk.
...
A direct calculation should show that this width coincides with that of \(\phi_-\).

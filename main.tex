...
After rescaling the free-energy density in Eq.~\ref{eq:fh_free_energy_density} by \(k_B T a^{-1}\), the model depends only on the Flory-Huggins interaction matrix \(\chi_{ij}\)
...
adsorption at the interface. This is because the accumulation of one species shifts the
profiles of the other species relative to each other along the x-axis.
Second, it ignores the intrinsic nonlinearity of the equilibrium profiles.
...
\begin{minipage}{0.35\textwidth}
    ...
    \captionof{figure}{(A) Schematic of the true interfacial profile (solid lines) compared with (B) the piecewise-linear approximation. The dashed lines in (A) denote mean compositions \(\bar\phi_i\).\\}
...
\[
\gamma \approx \sqrt{(\kappa_{ij}\Delta \phi_i \Delta \phi_j)\,(f(\bar\phi+\delta\phi_+) -2\mu_i( \bar{\phi} + \delta\phi_+)+ \Pi)}
\]
...
As predicted the main source of error after accounting for interfacial adsorption is the simplified functional form of the profile, which contributes a baseline error of approximately \(5\%\) as shown in Fig.~\ref{fig:fit_profiles}B.

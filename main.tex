...
\]
This relation is consistent with the numerical results shown in Fig.~\ref{fig:factor_scaling}A, where $\lvert \Delta \boldsymbol{\phi} \rvert^2$ is plotted against the chemical potential difference $\lvert \boldsymbol{\mu}-\boldsymbol{\mu}_c \rvert$ on a log-log scale.\\
...
properties analyzed here. The critical scaling of the interfacial width, \(w \sim |\boldsymbol{\mu}-\boldsymbol{\mu}_c|^{-1/2}\), agrees with the
diverging interfacial width observed in the numerical results \textcolor{red}{Add figure in supplimentary}, while the critical exponent of the surface tension, \(\gamma \sim |\boldsymbol{\mu}-\boldsymbol{\mu}_c|^{3/2}\), is consistent
